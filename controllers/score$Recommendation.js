...
exports.calculateScoresAndRecommendations = async (req, res) => {
	try {
		...
		const query = "SELECT question_id, category FROM questions";
		...
		const agreeablenessAssertivenessLearningTrack = recommendLearningTrack(
		...
		res.status(500).json({ error: "An error occurred while calculating scores.", message: error.message });
	}
};
...
function calculateOpennessScore(selectedOption) {
	...
	return optionScores[selectedOption] || 0;
}
...
function calculateImaginationScore(selectedOption) {
	...
	// if the score is undefined or null return 0
